...
- Optional safe mode deletes Telegram dialog messages after 7 full days without a real incoming or outgoing message.
- Every new incoming or confirmed outgoing message restarts the 7-day timer.
- Deletion is blocked while pending inbox, prepared crash-safe outbox, First DM delivery or a scheduled dialog action exists.
- Messages are deleted with revoke enabled when `DIALOG_AUTO_DELETE_FOR_BOTH=true`.
- Minimal identity, ownership, opt-out, First DM event and statistics metadata remains in SQLite.
- Existing databases are migrated in place and must not be cleared.
- Enable on Railway with `DIALOG_AUTO_DELETE_ENABLED=true`, `DIALOG_AUTO_DELETE_AFTER_DAYS=7` and `DIALOG_AUTO_DELETE_FOR_BOTH=true`.
