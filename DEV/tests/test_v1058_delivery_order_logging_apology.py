...
def test_first_dm_magnet_pool_is_valid(app_env):
    from services import ai_first_dm
    from texts.first_dm import MAGNET_FIRST_DM_TEMPLATES

    assert len(MAGNET_FIRST_DM_TEMPLATES) > 20
    for text in MAGNET_FIRST_DM_TEMPLATES:
        assert ai_first_dm.validate_first_dm(text, style="magnet")[0]
        assert ai_first_dm._MAGNET_TOPIC_RE.search(text)
