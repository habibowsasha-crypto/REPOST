...
def check_requirements() -> list[str]:
    errors: list[str] = []
    for filename in ("requirements.txt", "DEV/requirements-dev.txt"):
        path = ROOT / filename
        if not path.exists():
...
def main(argv: list[str] | None = None) -> int:
    ...
    ok, out = run_command([sys.executable, "DEV/scripts/import_check.py"])
    command_results.append(("imports", "PASS" if ok else "FAIL", out))

    if not args.skip_tests:
        ok, out = run_command([sys.executable, "-m", "pytest", "-q", "DEV/tests", "-o", "asyncio_mode=auto", "-p", "no:cacheprovider"])
