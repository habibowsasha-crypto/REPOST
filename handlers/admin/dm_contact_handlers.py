...
def _stats_text(stats: dict[str, int]) -> str:
    ...
    return (
        ...
        f"👥 Исторических пар «аккаунт + пользователь»: "
...
async def dm_contacts_clear_ask(event: callback_query) -> None:
    ...
    text = (
        ...
        "Пользователь снова сможет получить первый DM от любого подключённого аккаунта "
        "только после своего нового сообщения в отслеживаемом чате.\n\n"
...
async def dm_contacts_clear_yes(event: callback_query) -> None:
    ...
    text = (
        ...
        "Никому не отправлено сообщение. Новый первый DM от любого аккаунта возможен "
        "только после нового сообщения пользователя в отслеживаемом чате. "
        "Постоянный opt-out не изменён."
    )
    buttons = [
        [
            Button.inline(
