...
async def cmd_ai_status(event: callback_message) -> None:
    ...
    await render_menu(
        ...
        f"Бесплатный канал со сливами: софт копирует посты из {s.get('free_source_count', 6)} закрытых VIP-каналов | расширенный доступ: почти {s.get('paid_source_count', 50)}\n"
