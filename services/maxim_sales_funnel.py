...
def fallback_messages(
    ...
    *,
    ...
    if action == "transparent_offer":
        return [
            "Слушай, у меня есть доступ к бесплатной группе с випками, могу показать, если тебе интересно.",
            "Я вообще трафер: привлекаю туда людей за зарплату. Тебя никто не заставляет подписываться или что-то покупать.",
        ]
    ...
    if action == "business_model_link":
        return [
            f"Чел, я просто трафер за зарплату. Создатель бесплатной группы потом предлагает платный расширенный вариант — там уже почти {paid_count} VIP-источников, и СНГ, и западные.",
            f"В бесплатной группе ничего покупать не надо. Можешь просто глянуть и выйти, если не зайдёт: {PIRATE_VIP_LINK}",
        ]

    # link_offer
    return [
        f"Там сейчас уже {free_count} каналов известных трейдеров: они публикуют посты в закрытых группах, а программа копирует их почти моментально. Вот, глянь: {PIRATE_VIP_LINK}",
        "Ничего покупать не обязан — просто посмотри, вдруг тебе реально будет полезно.",
    ]
...
def _action_task(action: str, free_count: int, paid_count: int) -> str:
    tasks = {
        ...
        "transparent_offer": (
            "Сделай короткий переход к предложению. Скажи, что у Максима есть доступ к бесплатной группе "
            "и он может показать её. Вторым коротким сообщением честно скажи, что Максим трафер и получает "
            "зарплату за привлечение людей; человек ничего не обязан покупать или делать."
        ),
        "scam_reassurance": (
        ...
        "business_model_link": (
            "Прямо ответь, в чём выгода. Максим — трафер за зарплату. Создатель бесплатной группы позже "
            f"предлагает платный расширенный вариант почти с {paid_count} VIP-источниками, среди которых СНГ- и западные. "
            "Подчеркни, что бесплатная группа ни к чему не обязывает, и естественно дай точную ссылку."
        ),
        "link_offer": (
            "Объясни ценность и в этом же сообщении естественно дай ссылку без отдельного запроса разрешения. "
            f"Скажи, что в бесплатной группе уже {free_count} каналов известных трейдеров, их закрытые посты копируются "
            "программой почти моментально. Добавь, что можно просто посмотреть и ничего не покупать."
        ),
    }
    return tasks[action]
...
def _validate_generated(action: str, messages: list[str], free_count: int, paid_count: int) -> bool:
    ...
    if action == "pain_probe":
        if not any(marker in normalized for marker in ("жалко", "плат", "деньг")):
            return False
    if action == "transparent_offer":
        if "трафер" not in normalized or "зарплат" not in normalized:
            return False
    if action == "business_model_link":
        if str(paid_count) not in combined or "снг" not in normalized or "запад" not in normalized:
            return False
    if action == "link_offer":
        if str(free_count) not in combined or "почти момент" not in normalized:
            return False
    if action == "scam_reassurance":
...
async def generate_plan(
    *,
    ...
    instructions = f"""
Ты — Максим. Пиши как живой русскоязычный трафик-менеджер, а не как бот, гуру или трейдер-эксперт.
Твоя цель — поддержать уместный разговор, заинтересовать бесплатной подборкой и естественно привести к ссылке.
Ты честно говоришь, что привлекаешь людей за зарплату. Не скрывай бизнес-модель и не выдумывай опыт.
...
В бесплатной группе сейчас {free_count} каналов известных трейдеров. Публикации копируются почти моментально.
В платном расширенном варианте почти {paid_count} VIP-источников: СНГ и западные.
Текущий этап: {stage}
