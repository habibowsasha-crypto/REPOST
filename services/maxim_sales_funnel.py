...
# Soft close after link: no pressure + light apology for the ping.
AI_FIRST_DM_SOFT_CLOSE_VARIANTS: tuple[str, ...] = (
    "Просто глянь - не зайдёт, выйдешь. Извини, что побеспокоил.",
    "Можешь просто посмотреть, не понравится - выйди. Извини за беспокойство.",
    "Ни к чему не обязывает: глянул и ок. Извини, что отвлёк.",
    "Если не зайдёт - просто выйди, без обид. Извини, что написал.",
    "Там всё бесплатно, просто глянь. Не то - выйдешь. Извини за шум.",
    "Не давлю: посмотри по желанию. Извини, что побеспокоил.",
)
...
class FunnelPlan:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "messages",
            [_strip_long_dashes(str(x)) for x in (self.messages or [])],
        )
...
def _strip_long_dashes(text: str) -> str:
    """Replace em/en dashes with a plain hyphen; keep short '-'."""
...
def fallback_messages(
    ...
    *,
    ...
    if action == "identity_answer":
        return [
            "Я Максим. По сути помогаю людям находить бесплатный канал "
            "со сливами випок - без оплаты."
        ]
    if action == "vip_question":
        # Peer tip framing: free VIP-leaks via software, not a paid upsell.
        variants = [
            "Слушай, ты не видел канал, куда софт моментально кидает посты из закрытых випок? Бесплатно.",
            "А тебе попадались бесплатные каналы, где софт сразу копирует посты с закрытых випок?",
            "Кстати, видел такое: софт моментально дублирует посты из закрытых випок в один бесплатный канал?",
        ]
        return [random.choice(variants)]
    if action == "pain_probe":
        if re.search(r"\b(нет|не пробовал|никогда|не видел)\b", normalized):
            return [
                "Понял. Ну там как раз софт сам копирует посты с платных випок в один бесплатный канал - "
                "платить не нужно, просто смотришь."
            ]
        return [
            "Ага. Отдельно каждую випку брать обычно дорого. "
            "Тут софт моментально кидает посты в бесплатный канал - без оплаты."
        ]
    ...
    if action == "payment_answer":
        return [
            "Не, это бесплатно. Просто открываешь и смотришь, платить ничего не нужно."
        ]
...
def _action_task(action: str, free_count: int, paid_count: int, state: ConversationState) -> str:
    tasks = {
        ...
        "vip_question": "Отреагируй как свой из чата. Мягко спроси/подскажи про бесплатный канал, куда софт моментально копирует посты из закрытых випок. Не продавай и не спрашивай «платные или бесплатные варианты». Ссылку не давай.",
        "pain_probe": "Отреагируй по-человечески: отдельно випки брать дорого/непрозрачно, а софт моментально кидает посты в бесплатный канал - платить не нужно. Не дави. Ссылку не давай.",
        "project_explanation": f"Одним простым сообщением как свой из чата: есть бесплатный канал; софт моментально копирует туда новые посты из {free_count} закрытых випок; платить не нужно. Без «тарифов» и без зарплаты. Ссылку не давай.",
...
def build_ai_first_dm_short_plan(
    *,
    ...
    if followup_count >= max(1, int(max_followups) - 1):
        ...
        soft = random.choice(AI_FIRST_DM_SOFT_CLOSE_VARIANTS)
        return FunnelPlan(
            action="concise_link",
            next_stage="completed",
            close_after=True,
            messages=[
                f"{offer} {intro} {PIRATE_VIP_LINK}",
                soft,
            ],
            model="local_ai_first_dm",
        )

    # First reply after first DM → value + link + soft close.
    offer = random.choice(AI_FIRST_DM_OFFER_VARIANTS).format(
        from_bit=from_bit, free_count=free_count
    )
    intro = random.choice(AI_FIRST_DM_LINK_INTROS)
...
Не пиши длинное тире «—» или «–»; только обычный дефис «-».
Мягко снимай давление: «просто глянь», «не зайдёт - выйдешь», и коротко извинись за беспокойство, если уместно.
...
Бесплатный Telegram-канал со сливами VIP. Софт моментально копирует туда новые посты из {free_count} платных закрытых VIP-каналов трейдеров.
«Платные VIP» - источник контента, не товар на продажу.
Платить не нужно: просто смотришь в одном месте.

КАК ГОВОРИТЬ
- Тон: свой из чата, коротко, по-человечески.
- Всегда ясно: бесплатно, платить не нужно, софт копирует моментально.
- Не «платный или бесплатный вариант», не «попробуй випку», не презентация.
- Можно: «слушай», «глянь», «если не зайдёт - выйди».
