...
_LOCAL_FALLBACK = tuple(
    dict.fromkeys(
        ...
        + [
            "Привет 👋 Увидел тебя в чате, можно короткий вопрос?",
            "Салют, не отвлекаю? Хотел уточнить одну вещь по теме чата.",
            "Привет. Можно на минуту — вопрос по тому, что в чате обсуждают.",
            "Здорова 👋 Ты сейчас по теме чата или просто читаешь?",
            "Привет, видел твоё сообщение в чате. Можно спросить?",
        ]
    )
)
...
def _build_prompt(
    *,
    ...
    instructions = (
        ...
        "Нужно ровно ОДНО короткое сообщение на русском, 1–2 предложения, живой разговорный тон. "
        "Без ссылок, без рекламы, без обещаний прибыли, без VIP/оффера, без просьбы подписаться. "
        "Можно мягко опереться на то, что видел человека в чате. Не представляйся ботом."
    )
    user = (
        f"Контекст: человек писал в чате «{chat}». {name_hint}\n"
        "Задача: коротко поздороваться и задать нейтральный вопрос или завязать разговор по теме чата.\n"
