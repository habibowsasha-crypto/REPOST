...
_LOCAL_FALLBACK = tuple(
    dict.fromkeys(
        ...
        + [
            ...
            "Салют. Не отвлекаю? Хотел спросить одну вещь.",
            "Привет. Видел тебя в чате — можно короткий вопрос?",
            "Здорова 👋 Ты тут больше читаешь или иногда пишешь?",
            "Привет, можно на 10 секунд? Без воды.",
            "Эй, привет. Не для спама — просто один вопрос по чату.",
            "Салют. Если не занят — можно уточнить кое-что?",
            "Привет. Увидел твоё сообщение в чате, можно спросить?",
            "Здорова. Можно на минуту? Просто короткий вопрос.",
...
def _build_prompt(
    *,
    ...
    instructions = (
        ...
        "Цель — чтобы захотелось коротко ответить, как в обычной переписке. "
        "Ровно ОДНО короткое сообщение на русском, 1–2 предложения, максимум ~160 символов. "
        "Без ссылок, без рекламы, без VIP/оффера, без «подпишись», без обещаний прибыли. "
        "Не устраивай экзамен по теме чата (не спрашивай «что интересного в теме X»). "
        "Формула: видел в чате + лёгкий бытовой вопрос. Живой тон, как у своего."
    )
    user = (
        f"Контекст: человек писал в чате «{chat}». {name_hint}\n"
        "Задача: написать как свой из чата. Приветствие + «видел в чате» + лёгкий вопрос без экспертизы по теме. Чтобы хотелось просто ответить.\n"
