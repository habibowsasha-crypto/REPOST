...
_LOCAL_FALLBACK = (
    ...
    "Привет, помоги пожалуйста, есть вопрос",
    "Салют, не отвлекаю? Можно на секунду?",
    "Привет 👋 Можно короткий вопрос?",
    "Эй, привет. Есть секунда?",
    "Привет, можно уточнить кое-что?",
    "Здорова, не для спама - можно спросить?",
    "Привет, если не занят - один вопрос",
    "Салют, можно на 10 секунд?",
    "Привет, помоги плиз, коротко спросить можно?",
)
...
def _validate(message: str, recent: list[str]) -> tuple[bool, str]:
    ...
    if "—" in text or "–" in text:
        return False, "длинное тире"
    banned_survey = (
        "расслаб", "любимый", "после работ", "хобби", "о себе",
        "семь", "как дела", "чем заним", "расскажи", "активно участ",
        "интересного в теме", "твой опыт", "как относишься",
        "обмен мнени", "для информации или", "чаще читаешь или",
        "как думаешь", "скорее для информа",
        "сериал", "фильм", "кино", "смотрел", "смотрела", "смотрели",
        "музыка", "песн", "книга", "погода", "выходн", "отпуск",
        "из города", "сколько лет", "чем занима", "работаешь",
        "как настроен", "что думаешь о", "какой твой", "какая твоя",
        "какое твое", "какое твоё", "посоветуй", "рекоменд",
        "видел в чате, как ты", "видел тебя в чате", "увидел тебя в чате",
        "увидел в чате", "видел в чате, что ты", "как ты пишешь",
    )
    if any(marker in lower for marker in banned_survey):
        return False, "опросный/личный вопрос"
    if "?" in text:
        quiz_bits = (
            "какой ", "какая ", "какое ", "какие ", "что ", "где ", "когда ",
            "почему", "зачем", "сколько", "как тебе", "как ты ",
        )
        soft_ok = (
            "можно спросить", "можно уточнить", "можно на секунду",
            "можно короткий", "есть вопрос", "есть секунда",
            "не отвлекаю", "помоги",
        )
        if any(q in lower for q in quiz_bits) and not any(s in lower for s in soft_ok):
            return False, "вопрос-опросник"
...
def _build_prompt(
    *,
    ...
    instructions = (
        "Ты пишешь ПЕРВОЕ личное сообщение незнакомцу в Telegram. "
        "Только просьба разрешения спросить - как живой человек. "
        "ОДНА короткая фраза на русском, 4-12 слов, максимум 80 символов. "
        "Разрешено ТОЛЬКО в духе: «Привет, можно спросить?», "
        "«Привет, помоги пожалуйста, есть вопрос», «Салют, не отвлекаю? Можно на секунду?». "
        "СТРОГО ЗАПРЕЩЕНО: любой вопрос про сериал, фильм, музыку, хобби, работу, отдых, "
        "«любимый», «какой твой», «как ты пишешь», «видел в чате как ты…», опросы, анкеты. "
        "Без ссылок, VIP, оффера, эмодзи (или один максимум). Без длинного тире."
    )
    user = (
        f"Контекст: человек мелькал в чате «{chat}». {name_hint}\n"
        "Напиши ТОЛЬКО короткое «можно спросить?» по смыслу. Не задавай тематических вопросов.\n"
        "Примеры:\n"
        "- Привет, можно спросить?\n"
        "- Привет, помоги пожалуйста, есть вопрос\n"
        "- Салют, не отвлекаю? Можно на секунду?\n"
