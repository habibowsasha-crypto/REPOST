...
def _validate(message: str, recent: list[str]) -> tuple[bool, str]:
    ...
    if "-" in text or "-" in text:  # em/en dash; hyphen "-" is allowed
    ...
    banned_survey = (
        ...
        "как думаешь, это место", "скорее для информа",
    )
...
def _build_prompt(
    *,
    ...
    instructions = (
        ...
        "Пиши КАК ЖИВОЙ человек из чата: просто, коротко, чуть небрежно. "
        "Не как бот, не как маркетолог, не как опросник. "
        "Цель - чтобы ответили «да/а/ок». "
        "ОДНО короткое сообщение на русском, лучше 5-12 слов, максимум ~100 символов. "
        "Хороший тон: «Привет, можно спросить?», «Привет, помоги пожалуйста, есть вопрос», "
        "«Салют, не отвлекаю? Можно на секунду?». "
        "ЗАПРЕЩЕНО: шаблон «Увидел тебя в чате - как думаешь…», «место для обмена мнениями», "
        "«ты чаще читаешь или пишешь», «для информации или общения», длинные формулировки. "
        "ЗАПРЕЩЕНО: ссылки, VIP, оффер, реклама, прибыль, отдых, работа, хобби, «расскажи о себе». "
        "Можно слегка упомянуть чат, но без анкеты. Без эмодзи или максимум один. Не используй длинное тире «-» или «-», только обычный дефис «-» если нужен."
    )
    user = (
        f"Контекст: человек мелькал в чате «{chat}». {name_hint}\n"
        "Напиши живое первое DM как человек, не как ИИ.\n"
        "Примеры (не копируй дословно):\n"
        "- Привет, можно спросить?\n"
        "- Привет, помоги пожалуйста, есть вопрос\n"
        "- Салют, не отвлекаю? Можно на секунду?\n"
        "Не повторяй смысл этих недавних:\n"
        f"{recent_block}\n"
