...
_LOCAL_FALLBACK = (
    "Привет 👋 Увидел тебя в чате — можно на секунду?",
    "Салют. Не отвлекаю? Один короткий вопрос.",
    "Привет. Видел в чате — можно спросить?",
    "Здорова 👋 Ты тут больше читаешь или иногда пишешь?",
    "Привет, можно на 10 секунд? Без воды.",
    "Эй, привет. Не спам — просто один вопрос.",
    "Салют. Если не занят — можно уточнить кое-что?",
    "Привет. Увидел твоё сообщение в чате, ок спросить?",
    "Здорова. Можно коротко? Один вопрос.",
    "Привет 👋 Ты сейчас в чате или просто ленту листаешь?",
    "Салют, видел тебя в чате. Есть секунда?",
    "Привет. Не для рекламы — можно один вопрос?",
)
...
def _validate(message: str, recent: list[str]) -> tuple[bool, str]:
    ...
    banned_survey = (
        "расслаб", "любимый способ", "после работ", "хобби", "о себе",
...
def _build_prompt(
    *,
    ...
    instructions = (
        ...
        "Ты обычный человек из того же чата, не менеджер и не продавец. "
        "Цель — завлечь ответить одним коротким словом (да / а / ок / ну). "
        "Ровно ОДНО сообщение на русском: 1 предложение, лучше до 90 символов, максимум 140. "
        "Формула: короткое приветствие + «видел в чате» + ясный короткий вопрос-воронка. "
        "Вопрос должен быть понятный и лёгкий — без экспертизы и без личного. "
        "ЗАПРЕЩЕНО: ссылки, VIP, оффер, реклама, прибыль, винрейт. "
        "ЗАПРЕЩЕНО: отдых, работа, хобби, семья, «любимый способ», «расскажи о себе», "
        "«ты активно участвуешь», длинные формулировки, экзамен по теме чата. "
        "Можно: «можно на секунду?», «ты тут читаешь или пишешь?», «есть секунда?», «ок спросить?»."
    )
    user = (
        f"Контекст: человек мелькал в чате «{chat}». {name_hint}\n"
        "Напиши завлекающее первое DM: короткий хук + простой вопрос, на который легко ответить.\n"
        "Примеры тона (не копируй дословно):\n"
        "- Привет 👋 Увидел тебя в чате — можно на секунду?\n"
        "- Салют. Не отвлекаю? Один короткий вопрос.\n"
        "- Привет. Ты тут больше читаешь или иногда пишешь?\n"
        "Не повторяй смысл этих недавних сообщений:\n"
        f"{recent_block}\n"
