...
## v1.0.2 — Menu & FSM hardening

Главное меню теперь содержит кнопки DM и AI:

- `💬 Запустить DM`
- `📋 DM-задачи`
- `🛑 Остановить DM-задачу`
- `🤖 AI статус`
- `💬 AI-диалоги`
- `📝 Первые DM-шаблоны`

Добавлена команда `/cancel` и кнопка `✖️ Сбросить текущий ввод`.
Любая slash-команда больше не воспринимается старыми мастерами ввода как текст или число.
Команда `/start` сбрасывает незавершённые мастера и показывает полное меню.


## Меню и синхронизация групп (v1.0.3)

- Главное меню открывается командами `/start`, `/menu` или обычным сообщением `Меню`.
- Нажатия inline-кнопок обновляют текущее сообщение, а не создают новое.
- В карточке подключённого аккаунта есть кнопка **«🔎 Найти группы аккаунта»**.
- Синхронизация показывает публичные и закрытые группы, сохраняет их доступность и служебные Telegram-данные.
- В рабочий список автоматически включаются только группы, которыми подключённый аккаунт управляет. Остальные отображаются только для просмотра.
- Новые переменные Railway для этой версии не нужны.


## v1.0.4: общий каталог и рабочие группы

- «Мои группы» показывает общий каталог (`pre_groups`).
- Рабочие группы принадлежат конкретному подключённому аккаунту (`groups`).
- «Найти группы аккаунта» показывает все доступные группы.
- В рабочий список можно включить только доступную группу, которой аккаунт управляет.
- Закрытые группы сохраняются с `access_hash` в `discovered_groups`.
- Группы только для просмотра видны в интерфейсе, но не используются DM-мониторингом.
