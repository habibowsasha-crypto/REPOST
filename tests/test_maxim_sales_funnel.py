...
class MaximSalesFunnelTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_context_funnel_reaches_link(self) -> None:
        ...
        self.assertTrue(any(x in self.client.sent[-1].lower() for x in ("жалко", "дорого", "бесплат", "софт", "платит")))
    ...
    async def test_followup_cap_uses_one_final_link_reply(self) -> None:
        history = [
            ...
            ("incoming", "Да"),
            ("outgoing", "Слушай, ты не видел канал, куда софт моментально кидает посты из закрытых випок? Бесплатно."),
            ("incoming", "Нет"),
            (
                "outgoing",
    ...
    async def test_identity_question_gets_simple_maxim_answer(self) -> None:
        ...
        self.assertTrue(
            any(x in combined for x in ("привлеч", "помогаю", "бесплат", "вип"))
        )
