...
from services.spambot_monitor import get_spambot_monitor_state, set_spambot_monitor_enabled
...
class _NeverCompletesClient(_FakeClient):
    async def send_message(self, peer, text: str) -> None:
...
class SafeQueueTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        with conn:
            ...
            try:
                conn.execute(
                    "UPDATE dm_queue_runtime SET mode='per_account', "
    ...
    def test_stale_claim_recovery_is_conservative(self) -> None:
        ...
        self.assertEqual(result["sending_uncertain"], 1)
    ...
    def test_process_start_recovers_fresh_claim_and_protects_fresh_send(self) -> None:
        ...
        self.assertEqual(normal["claimed_recovered"], 0)
        self.assertEqual(normal["sending_uncertain"], 0)
        startup = recover_stale_queue(process_start=True)
        self.assertEqual(startup["claimed_recovered"], 1)
        self.assertEqual(startup["sending_uncertain"], 1)
        rows = {
    ...
    async def test_peerflood_pauses_account_until_manual_resume(self) -> None:
        ...
        set_spambot_monitor_enabled(110, True)
        ...
        monitor = get_spambot_monitor_state(110)
        self.assertEqual(monitor.status, "pending")
        self.assertIsNotNone(monitor.next_check_at)
    ...
    async def test_telegram_send_timeout_is_marked_uncertain(self) -> None:
        self._task(task_id=1, account=121)
        _, row_id = self._enqueue(task_id=1, account=121, user=522, chat=11)
        dm_handlers.dm_monitor_clients[1] = _NeverCompletesClient()
        with patch.object(dm_handlers, "_telegram_send_timeout_seconds", return_value=0.02):
            result = await dm_handlers._send_pending_row(get_due_pending(121))
        self.assertEqual(result, "uncertain")
        status, error = conn.execute(
            "SELECT status,last_error FROM dm_pending_queue WHERE id=?", (row_id,)
        ).fetchone()
        self.assertEqual(status, "uncertain_delivery")
        self.assertIn("telegram_send_timeout", error or "")
        self.assertIsNone(get_due_pending(121))
