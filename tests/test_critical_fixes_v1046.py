...
def test_pause_gate_exists_only_in_first_dm_worker(app_env):
    src = Path("services/dialog_engine.py").read_text(encoding="utf-8")
    auto_part = src.split("async def process_due_auto_links", 1)[1]
    assert "is_worker_enabled" not in auto_part
    assert "Main-menu pause stops only new First DM" in auto_part
